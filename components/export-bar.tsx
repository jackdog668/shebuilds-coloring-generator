...
import { buildListing } from "@/lib/coloring/listing-builder";
import { encodeState } from "@/lib/coloring/url-state";
...
export function ExportBar({ state }: Props) {
  ...
  const handleCopyPrompt = () =>
    withStatus(setPromptStatus, async () => {
      const p = buildColoringPrompt(state);
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <BrandButton variant="outline" onClick={handleCopyListing} disabled={listingStatus === "working"}>
        <StatusLine status={listingStatus} icon={<FileText className="h-4 w-4" />} idleLabel="Copy KDP listing" doneLabel="Copied" />
      </BrandButton>

      <BrandButton variant="outline" onClick={handleCopyTags} disabled={tagsStatus === "working"}>
        <StatusLine status={tagsStatus} icon={<TagIcon className="h-4 w-4" />} idleLabel="Keywords" doneLabel="Copied" />
      </BrandButton>
