...
import { z } from "zod";
import { checkRateLimit, ipFromRequest } from "@/lib/coloring/rate-limit";
...
export async function POST(req: Request) {
  ...
  if (!limit.ok) {
    ...
    return NextResponse.json(
      {
        error: `Free tier limit reached: ${RATE_LIMIT_PER_HOUR} AI generations per hour. Copy the prompt below and paste it into your favorite AI tool — or come back in a bit.`,
        retryAfter,
      },
      {
        status: 429,
